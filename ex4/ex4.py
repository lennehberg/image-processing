...
def visualize_lucas_kanade(frame_a, frame_b, step=16):
    """
    Visualizes the Lucas-Kanade optical flow between two frames.

    :param frame_a: The first grayscale frame.
    :param frame_b: The second grayscale frame.
    :param step: Step size for the grid of flow vectors to visualize.
    """
    # Ensure the frames are grayscale
    ...
    flow = cv2.calcOpticalFlowFarneback(frame_a, frame_b, None,
    ...
    # Create an image to visualize flow vectors
    plt.figure(figsize=(10, 6))
    plt.imshow(frame_a, cmap='gray')
    plt.quiver(x, y, fx, fy, color='r', scale=50, angles='xy')
...
def get_strip(aligned_frame, strip_center, l_strip_width, r_strip_width):
    ...
    # print(start, end)
    count += 1
    display_strip_with_box(aligned_frame, start, end)
    return aligned_frame[:, start: end, :]
...
def make_canvas(vid):
    ...
    for i in range(1, len(vid)):
        # visualize_lucas_kanade(grayscale_vid[i - 1], grayscale_vid[i])
